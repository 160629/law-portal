	...
	function checkIsRequired(component){
		if(component.type=='checkbox'){
			if(!component.checked){
				alertMessage(component,"Please choice necessarily option");
				component.focus();
				return false;
			}
		}else{
			if(isEmpty(component.value)){
				alertMessage(component,"Please input must value ");
				component.focus();
				return false;
			}
		}
	...
	function checkNumber(component){
		if(!isCharsInBag(component.value,numberlist)){
			alertMessage(component,"Please input a value of a digital type ");
			component.focus();
			component.select();
			return false;
			...
			alertMessage(component,"Beyond the accuracy range [999999999.99999] ");
			component.focus();
			component.select();
			return false;
		}
	...
	function checkMail(component){
		if(!checkMailAddr(component.value)){
			alertMessage(component,"Email address is not correct ");
			component.focus();
			component.select();
			return false;
		}
