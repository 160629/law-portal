...
//cal = new AutoFormCalendar(by, ey,1,strFormat); //初始化英文版，0 为中文版 
cal = (cal==null) ? new AutoFormCalendar(by, ey, 1) : cal; //不用每次都初始化  修正 
...
//返回所选日期 
AutoFormCalendar.prototype.ReturnDate = function(dt) { 
...
* @update 
*/ 
function AutoFormCalendar(beginYear, endYear, lang, dateFormatStyle) { 
this.beginYear = 1950; 
this.endYear = 2050; 
...
AutoFormCalendar.prototype.draw = function() { 
calendar = this; 

var mvAry = []; 
//mvAry[mvAry.length] = ' <form name="calendarForm" style="margin: 0px;">'; //因 <form> 不能嵌套， 改用 Div 
...
mvAry[mvAry.length] = ' <tr>'; 
for(var i = 0; i < 7; i++) { 
mvAry[mvAry.length] = ' <th style="font-weight:normal;background-color:' + calendar.colors["tr_bg"] + ';color:' + calendar.colors["tr_word"] + ';">' + AutoFormCalendar.language["weeks"][this.lang][i] + '</th>'; 
} 
mvAry[mvAry.length] = ' </tr>'; 
for(var i = 0; i < 6;i++){ 
...
mvAry[mvAry.length] = ' <select name="calendarHour" id="calendarHour" style="font-size:12px;"></select>' + AutoFormCalendar.language["hour"][this.lang]; 
mvAry[mvAry.length] = '<span style="' 
if(calendar.DateMode >= pickMode["hour"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时时隐藏“分” 
mvAry[mvAry.length] = '"><select name="calendarMinute" id="calendarMinute" style="font-size:12px;"></select>' + AutoFormCalendar.language["minute"][this.lang]+'</span>'; 
mvAry[mvAry.length] = '<span style="' 
if(calendar.DateMode >= pickMode["minute"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到小时、分时隐藏“秒” 
mvAry[mvAry.length] = '"><select name="calendarSecond" id="calendarSecond" style="font-size:12px;"></select>'+ AutoFormCalendar.language["second"][this.lang]+'</span>'; 
...
mvAry[mvAry.length] = ' <input name="calendarClear" type="button" id="calendarClear" value="' + AutoFormCalendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer;"/>'; 
mvAry[mvAry.length] = ' <input name="calendarToday" type="button" id="calendarToday" value="' 
mvAry[mvAry.length] = (calendar.DateMode == pickMode["day"]) ? AutoFormCalendar.language["today"][this.lang] : AutoFormCalendar.language["pickTxt"][this.lang]; 
...
mvAry[mvAry.length] = ' <input name="calendarClose" type="button" id="calendarClose" value="' + AutoFormCalendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:40px;height:20px;font-size:12px;cursor:pointer"/>'; 
...
{ calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*/// 注释 
} 
...
//年份下拉框绑定数据 
AutoFormCalendar.prototype.bindYear = function() { 
var cy = this.calendarYear; 
cy.length = 0; 
for (var i = this.beginYear; i <= this.endYear; i++){ 
cy.options[cy.length] = new Option(i + AutoFormCalendar.language["year"][this.lang], i); 
} 
} 

//月份下拉框绑定数据 
AutoFormCalendar.prototype.bindMonth = function() { 
var cm = this.calendarMonth;
cm.length = 0; 
for (var i = 0; i < 12; i++){ 
cm.options[cm.length] = new Option(AutoFormCalendar.language["months"][this.lang][i], i); 
} 
} 

//小时下拉框绑定数据 
AutoFormCalendar.prototype.bindHour = function() { 
...
//分钟下拉框绑定数据 
AutoFormCalendar.prototype.bindMinute = function() { 
...
//秒钟下拉框绑定数据 
AutoFormCalendar.prototype.bindSecond = function() { 
...
//向前一月 
AutoFormCalendar.prototype.goPrevMonth = function(e){ 
...
//向后一月 
AutoFormCalendar.prototype.goNextMonth = function(e){ 
...
//改变SELECT选中状态 
AutoFormCalendar.prototype.changeSelect = function() { 
var cy = this.calendarYear;
var cm = this.calendarMonth; 
var ch = this.calendarHour; 
var cM = this.calendarMinute; 
...
//更新年、月 
AutoFormCalendar.prototype.update = function (e){ 
...
//绑定数据到月视图 
AutoFormCalendar.prototype.bindData = function () { 
var calendar = this; 
...
//根据年、月得到月视图数据(数组形式) 
AutoFormCalendar.prototype.getMonthViewArray = function (y, m) { 
var mvArray = []; 
...
AutoFormCalendar.prototype.getElementById = function(id){ 
...
AutoFormCalendar.prototype.getElementsByTagName = function(object, tagName){ 
if (document.getElementsByTagName) return document.getElementsByTagName(tagName); 
if (document.all) return document.all.tags(tagName); 
} 

//取得HTML控件绝对位置 
AutoFormCalendar.prototype.getAbsPoint = function (e){ 
var x = e.offsetLeft; 
var y = e.offsetTop; 
...
//显示日历 
AutoFormCalendar.prototype.show = function (dateObj, popControl) { 
if (dateObj == null){ 
...
if(this.panel.innerHTML==""||cal.dateFormatStyleOld != cal.dateFormatStyle)// 把构造表格放在此处， 若请示的样式改变，则重新初始化 
{ 
this.draw(); 
this.bindYear(); 
this.bindMonth(); 
...
//把 visibility 变为 display，并失去焦点的事件 //this.setDisplayStyle("select", "hidden"); 
...
//隐藏日历 
AutoFormCalendar.prototype.hide = function() { 
...
//焦点转移时隐藏日历  
AutoFormCalendar.prototype.onblur = function() { 
if(!isFocus){this.hide();} 
} 

//修改 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方 
...
obj = this.getElementById("calendarClear");
obj.onclick = function () 
{ calendar.ReturnDate(""); /*calendar.dateControl.value = "";calendar.hide();*/// 由注释
}
this.calendarClear = obj;
...
//this.panel.style.left = xy.x + "px";
//this.panel.style.top = (xy.y + dateObj.offsetHeight) + "px";
this.panel.style.left = (xy.x + leftX)+ "px"; //由  修改 → 加入自定义偏移量
this.panel.style.top = (xy.y + topY + dateObj.offsetHeight) + "px";

//由  修改 → 把 visibility 变为 display，并失去焦点的事件 //this.setDisplayStyle("select", "hidden");
//this.panel.style.visibility = "visible";
//this.container.style.visibility = "visible";
this.panel.style.display = "";
this.container.style.display = "";
...
function InitContainerPanel() //初始化容器
{
var str = '<div id="calendarPanel" style="position: absolute;display: none;z-index:9999; background-color: #FFFFFF;border: 1px solid #CCCCCC;width:176px;font-size:12px;"></div>';
if(document.all)
{
str += '<iframe style="position:absolute;z-index:2000;width:expression(this.previousSibling.offsetWidth);';
str += 'height:expression(this.previousSibling.offsetHeight);';
