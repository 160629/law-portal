	function setNodeIcon(node,type){
		if(type=="emp"||type=="org"||type=="organization"||type=="person"||type=="position"||type=="position_list"||type=="role"||type=="role_view"){
			node.setIcon("/workflow/wfcomponent/web/images/participant/"+type+".gif");
		}else{
			node.setIcon("/workflow/wfcomponent/web/images/participant/default.gif");
		}
	}
	...
	*/
	...
	{	
		...
		this.set = function (id,value) {
			if (!isExist(id)) {
				throw new Error("Specified data buffering ID:"+id+"not exit");//指定的数据缓存ID:/不存在!
				return false;
			}
			for (var i=0;i<dataCatchPool.length;++i) {
					dcatch = dataCatchPool[i] ;
...
function checkAll(f) 
	...
	function getSid (f) 
	{
		...
		for (var i=0; i<length; i++){ 
		  ...
		  fids = cv;
		  if (num == 0) {
		  	alert(have_not_dataitem_use);//当前无数据项可用
		  	return false ;
		  }
		  
		  if (cv.length<1) {
		  	   alert(select_operation_data_item);//请先选择要操作的数据项
		  	   return false ;
		  }
...
function f_check_bpscommname(obj){
	...
	if(isBPSNameFormat(str)){
		...
		f_alert(obj,format_error1);//格式错误，应仅含中文、字母、数字、下划线、<br/>&nbsp;英文符号【-()[]{}.:】、中文符号【，。：‘’“”】
        return false;
	}
}
...
function f_check_bpscommimpl(obj){
	...
	if(isBPSImplFormat(str)){
		...
		f_alert(obj,format_error2);//格式错误，应仅含中文、字母、数字、下划线、<br/>&nbsp;英文符号【-()[]{}.:/<>$】、中文符号【，。：‘’“”】
        return false;
	}
}
...
function f_check_bpscommxpath(obj){
	...
	if(isBPSXpathFormat(str)){
		...
		f_alert(obj,format_error3);//格式错误，应仅含中文、字母、数字、下划线、<br/>&nbsp;英文符号【-()[]{}.:/】、中文符号【，。：‘’“”】
        return false;
	}
}
...
function f_check_bpscommxml(obj){
	...
	if(isXMLFormat(str)){
		...
		f_alert(obj,format_error4);//格式错误，不能包含特殊符号【<>】
        return false;
	}
}

//判断是否颜色校验规则
//请检查一下颜色值，是否是#+六位十六进制数，比如：#A52A2A
function isBPSLinkColorFormat(str){
    var reg = /^#[0-9a-fA-F]{6}$/;
    if(reg.test(str)){
    	return true;
    }else{
...
//使用在eos校验标签内，在validateAttr="type=bpslinkcolor"
function f_check_bpslinkcolor(obj){
	var str = obj.value;
	if(isBPSLinkColorFormat(str)){
		return true;
	}else{
		f_alert(obj,format_error5);//格式错误，请检查一下颜色值，是否是#+六位十六进制数，比如：#A52A2A
        return false;
	}
}
