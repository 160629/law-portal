...
Element.extend({
	...
	clone : function (C) {
		var B = $mootools(this.cloneNode(C !== false));
		if (!B.$events) {
			return B;
		}
		B.$events = {};
	...
	getParent : function () {
		return $mootools(this.parentNode);
	},
...
Drag.Base = new Class({
		...
		initialize : function (B, A) {
			this.setOptions(A);
			this.element = $mootools(B);
			this.handle = $mootools(this.options.handle) || this.element;
			this.mouse = {
				now : {},
				pos : {}
...
Drag.Move = Drag.Base.extend({
		...
		initialize : function (B, A) {
			...
			this.container = $mootools(this.options.container);
			this.position = {
