...
$(function () {
    //列表控件事件加载
    selectTypes(0);//   待阅类型
    $("#updateStatus").on("click",updateStatusFun);//设为已阅
...
//结束委托
function updateStatusFun() {
    var idList = [];
    $("#bus-table input[type='checkbox']:checked").each(function () {
        idList.push($(this).val());
        updateStatusDataFun(idList);//执行删除请求
    });
}

function updateStatusDataFun(idList) {
    ajax_req({
        ...
        success: function (rs) {
            if (rs.resultStat == "SUCCESS") {
                paging("pageContainer", "loadData", paramObj);//加载分页数据绑定到列表
                layer.msg("设置已阅成功", {time: 1000},function () {
                    loadData(paramObj);//tab标上加载数量
                });
            } else {
                layer.alert('设置已阅失败，请联系管理员', {
                    icon: 2,
                    title: "提示"
                });
            }
            ...
            if (rs.resultStat == "SUCCESS") {
                ...
                if (rs.data != null) {
                    ...
                    for (var index in rs.data.list) {
                        ...
                        html += "<tr>" +
                            "<td><span><input type='checkbox' value='" + data.viewId + "' name='checkbox' class='checkboxipt' /></span><span class='index'>" + (i++) + "</span></td>" +
                ...
                // $("#task_total").html(pageData.count);//加载数量
                if (!html){
