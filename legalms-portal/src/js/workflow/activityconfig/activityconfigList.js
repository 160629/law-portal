...
function queryBtnFun(e) {
    ...
    if (paramObj.importantLevel == "简单支撑决策") {
        paramObj.importantLevel = 'share'
    } else if (paramObj.importantLevel == "一般支撑决策") {
        paramObj.importantLevel = 'normal'
    } else if (paramObj.importantLevel == "重大支撑决策") {
        paramObj.importantLevel = 'commonly'
    }
...
//     param.pageNum = 1;
//     ajax_req({
//         url: baseUrl.activityconfig.selectOrgInActivityconfig,
//         type: 'post',
...
function delDataFun(idList) {
    ...
    layer.confirm("确定删除选中的信息吗？", { btn: ['确定', '取消'], title: "提示" }, function () {
        ajax_req({
            url: baseUrl.activityconfig.deleteActivityconfig,
            type: 'post',
...
function jumpDetails() {
    var path = pageUrl.activityconfig.activityconfigEdit;
...
jumpDetails("&flowId='com.tower.empower.city'&id='1'");
setTimeout(function(){
