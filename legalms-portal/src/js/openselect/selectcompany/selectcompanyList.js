...
function child(paramData) {
    ...
    $('#alertSubmitBtn').on('click', function () {
        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
        var $companys = $(".checkedlist li");
        if ($companys.length > 0) {
            var array = [];
            $.each($companys, function (i, $li) {
    ...
    $('tbody').on("click", 'tr', function () {
        ...
        if (!checkbox.is(":checked")) {
            ...
            if (paramData.isRadio) {
                ...
                appendCompany();//设置选人为空
            }
            var data = checkbox.next().val();
            appendCompany(eval("(" + data + ")"));
    ...
    if (paramData.selectItem){
        ...
        if (data.bodyCode){
            var bodyCodeS = data.bodyCode;
            var objArray=[];
            //切出所有需要回填的所有存储数组对象的数组
            $.each(data, function (i, value) {
                if (value) {
                    var valArray = value;
                    $.each(bodyCodeS, function (j, value) {
                        //用主键数量控制循环重新组装逻辑
...
function loadData(param) {
    ...
    ajax_req({
        url: baseUrl.SysOurmainbody.selectSysOurmainbody,
        type: 'post',
        async: false,
            ...
            if (rs.resultStat == "SUCCESS") {
                ...
                if (rs.data != null) {
                    ...
                    for (var index in rs.data.list) {
                        ...
                        var trSelect = "";
                        if ($("#" + data.bodyCode + "_data").length > 0) {
                            trSelect = 'tr-select';
                        }
                        html += "<tr class=" + trSelect + ">" +
                            "<td><span><input type='checkbox' id='" + data.bodyCode + "' value='" + data.bodyName + "' name='checkbox' class='checkboxipt hide' />" +
                            "<input type='hidden' value='" + JSON.stringify(data) + "'></span><span class='index'>" + (i++) + "</span></td>" +
                            "<td><a href='#' class='title-link'>" + data.bodyName + "</a></td>" +
                            "</tr>";
                    }
                }
                if (!html){
...
//将已选择添加到已选列表
function appendCompany(param) {
    if (null != param) {
        if ($("#" + param.bodyCode + "_data").length == 0) {
            var checkHtml = "<li id='" + param.bodyCode + "_data'><input type='hidden' value='"+JSON.stringify(param)+"'><span class='text'>" + param.bodyName + "</span><div class='option-icon'><i class='up layui-icon layui-icon-up'></i><i class='down layui-icon layui-icon-down'></i><i class='del layui-icon layui-icon-delete'></i></div></li>";
            $(".checkedlist").append(checkHtml);
        }else{
            $("#" + param.bodyCode + "_data").remove()
        }
    } else {
        $(".checkedlist").html("");
    }
