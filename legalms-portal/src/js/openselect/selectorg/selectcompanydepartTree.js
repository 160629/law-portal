...
function child(paramData) {
    ...
    companyOrgLog(paramData.param);//初始化公司信息

    //当前页提交响应事件，执行父页面回调函数
    $('#alertSubmitBtn').on('click', function () {
        var index = parent.layer.getFrameIndex(window.name); //获取窗口索引
        var $companyOrgs = $(".checkedlist li");
        if ($companyOrgs.length > 0) {
            var array = [];
            $.each($companyOrgs, function (i, $li) {
                var value = $($li).find("input[type=hidden]").val();
                array.push(eval("(" + value + ")"));
            });
            paramData.callback(array);//返回选中部门，由表单启动流程
            parent.layer.close(index);
        } else {
...
function zTreeOnClick(e, treeId, treeNode) {
    if (setting.check.isRadio) {
        ...
        appendcompanyOrg();//设置选人为空
    }
    appendcompanyOrg(treeNode);
}

//从节点人员中【拼接处理】后添加到已选人员
function appendcompanyOrg(treeNode) {
    if (null != treeNode){
        ...
        if ($("#" + treeNode.id+ "_data").length == 0) {
            ...
            var checkHtml = "<li id='" + treeNode.id + "_data'><input type='hidden' value='"+JSON.stringify(companyOrgData)+"'><span class='text'>" + companyOrgData.val + "</span><div class='option-icon'><i class='up layui-icon layui-icon-up'></i><i class='down layui-icon layui-icon-down'></i><i class='del layui-icon layui-icon-delete'></i></div></li>";
