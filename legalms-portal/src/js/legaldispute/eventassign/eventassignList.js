...
function loadData(param) {
    ...
    ajax_req({
        url: baseUrl.caseAssign.caseAssignList,
        type: 'post',
        dataType: 'json',
            ...
            if (rs.resultStat == "SUCCESS") {
                ...
                if (rs.data != null) {
                    ...
                    for (var index in rs.data.list) {
                        ...
                        html += "<tr>" +
                            "<td><span><input type='checkbox' status='" + data.assignStatus + "'value='" + data.assignId + "' name='checkbox' class='checkboxipt' /></span><span class='index'>" + (i++) + "</span></td>" +
                            "<td><a href='#' class='title-link' id='" + data.assignId + "'moduleName='" + data.moduleName + "' status='" + data.assignStatus
...
function delDataFun(idList) {
    layer.confirm("确定删除选中的信息吗？",{btn: ['确定', '取消'], title: "提示"},function () {
        ajax_req({
            url: baseUrl.caseAssign.deleteTCaseAssign,
            type:'post',
