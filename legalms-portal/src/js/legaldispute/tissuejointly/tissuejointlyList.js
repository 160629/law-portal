...
function queryBtnFun(e) {
    ...
    if (e.data == "status") {
        paramObj.jointlyStatus = $(e.currentTarget).attr("val");
    }
...
function loadData(param) {
    ...
    ajax_req({
        url: baseUrl.tIssueJointly.findTIssueJointly,
        type: 'post',
        async: false,
            ...
            if (rs.resultStat == "SUCCESS") {
                ...
                if (rs.data != null) {
                    ...
                    for (var index in rs.data.list) {
                        ...
                        html += "<tr>" +
                            "<td><span><input type='checkbox' value='" + data.jointlyId + "' status='" + data.jointlyStatus + "' name='checkbox' class='checkboxipt' /></span><span class='index'>" + (i++) + "</span></td>" +
                            "<td><a href='#' class='title-link' id='" + data.jointlyId + "' status='" + data.jointlyStatus+ "'moduleName='" + data.moduleName
                            + "'title='" + data.jointlyTitle + "'>" + data.jointlyTitle + "</a></td>" +
...
function delDataFun(idList) {
    layer.confirm("确定删除选中的信息吗？",{btn: ['确定', '取消'], title: "提示"},function () {
        ajax_req({
            url: baseUrl.tIssueJointly.deleteTIssueJointly,
            type:'post',
            dataType: 'json',
