    ...
    var formInfo = {
            ...
            checkUnit: function () {
                ...
                if (userInfo.CURRUSERUNITID != jointlyUnitId) {
                    $("#w_jointlyCase").find("input[name=jointlyOrg]").removeAttr("lay-verify").parents(".layui-row").find("b").html("");
                } else {
                    $("#w_jointlyCase").find("input[name=jointlyOrg]").attr("lay-verify", "required").parents(".layui-row").find("b").html("*");
                }
            },
            //协办单位弹出
                    ...
                    openSelectWindow.selectorgTree({
                        ...
                        callback: function (res) {
                            if (jointlyUnitId != res[0].id) {
                                $("#w_jointlyCase").find("input[name=jointlyOrg]").val("");
                                $("#w_jointlyCase").find("input[name=jointlyOrgId]").val("");
                            }
                            $parent.find("input[name=unitLevel]").val(res[0].orgLevel);
                            $parent.find("input[name=jointlyUnitName]").val(res[0].val);
                            $parent.find("input[name=jointlyUnitId]").val(res[0].id);
                            formInfo.checkUnit()
                        }
                    }, {
                        title: '请选择执行单位'
            ...
            writeFormBaseInfo: function () {
                ...
                $.extend(viewObj, {
                    ...
                    caseRecordTime: dateFormat(formInfo.formObj.caseMainVO.caseRecordTime, 'yyyy-MM-dd'),
                    ...
                    deputeType: dataDict.getDictValueByKey("sys_case_type", formInfo.formObj.caseMainVO.deputeType),
                    caseSpecialLine: dataDict.getDictValueByKey("sys_case_line", formInfo.formObj.caseMainVO.caseSpecialLine),
                    caseReviewGrade: dataDict.getDictValueByKey("sys_case_level", formInfo.formObj.caseMainVO.caseReviewGrade),
                    largeLawsuitMark: dataDict.getDictValueByKey("sys_issue_size", formInfo.formObj.caseMainVO.largeLawsuitMark),
                    ourLawsuitIdentity: dataDict.getDictValueByKey("sys_our_litigation", formInfo.formObj.caseMainVO.ourLawsuitIdentity)
                });

                $("input[name=caseDeputeMoney]").next(".aux").html(common.convertCurrency(formInfo.formObj.caseMainVO.caseDeputeMoney));//金额数字转大写
                ...
                formInfo.driveWhereParam.jointlyOrgId = formInfo.formObj.jointlyOrgId;//将自己协办部门id赋值
            ...
            backBtnFun: function () {
                formInfo.formCheck(function ($form) {
                    ...
                    callback({
                        notRequiredField: {jointlyUnitName:true,jointlyOrg:true,jointlyItem:true,feedBack: true}
                    });
                });
            },
            //下一步
                    ...
                    callback({
                        form_verify:{
                            unitLevel :function (value, item) {
                                ...
                                var unitLevel = $("#w_jointlyCase").find("input[name=unitLevel]").val();
                                if (unitLevel) {
                                    //判断如果选择单位不是当前组织级别则去掉部门必选规则
                                    var userInfo = getUserData('pid');
            ...
            getFlowingInfo: function () {
                ...
                if (business_form.jointlyUnitId) {
                    formInfo.driveWhereParam.unitCode = business_form.jointlyUnitId;//将自己协办单位id赋值
                }

                if (business_form.jointlyOrgId) {
                    formInfo.driveWhereParam.jointlyOrgId = business_form.jointlyOrgId;//将自己协办部门id赋值给它
                }
                return infoObj;
            },
            //获取退回上一步流程所需参数对象
            ...
            getNextFlowInfo: function (nextNode, flowingInfo) {
                ...
                if (nextNode.actDefParam == "finishActivity") {
                    ...
                    model.busCode = formInfo.formObj.jointlyCode;
                    model.busTitle = formInfo.formObj.jointlyTitle;
                }
                return model;
            },
            //取地址栏参数
            ...
            bindEvent: function () {
                //文件上传
                $("#thelist_jointlyFile_w").find('input[type="file"]').on('change', function () {
                    uploadFile.upload($("#thelist_jointlyFile_w"), this, {shareType: 'jointlyFile', formId: formInfo.formId});
                });
            ...
            initFormWrite: function () {
                ...
                if (!isEmpty(formInfo.formId)) {
                    ...
                    if (!isEmpty(resourceData)) {
                        ...
                        uploadFile.writeFileInfo($("#thelist_jointlyFile_r"), formInfo.formObj.files, {
                            onDel: false,
                            shareType: 'jointlyFile'
                        });
                        uploadFile.writeFileInfo($("#thelist_jointlyFile_w"), formInfo.formObj.files, {
                            shareType: 'jointlyFile',
